...
- Fetch from `https://github.com/ash-lang/ash/releases/latest/download/<filename>`
...
- `tasks/backlog/github.md` is a prerequisite task covering CI/CD and GitHub Releases. The `postinstall.js` download URL depends on GitHub Releases existing.
...
- The download filename pattern should match the CI Release asset naming (see `tasks/backlog/github.md`).
...
- CI workflows (from `tasks/backlog/github.md`) are owned by that task; this task only creates the npm publishing artifacts the CI will consume.
