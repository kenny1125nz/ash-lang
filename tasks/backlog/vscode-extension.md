...
**Prerequisite tasks** (in backlog):
- `github.md` — CI/CD pipeline that builds all platform binaries (the extension's `bin/` dir comes from these builds)
- `npm-package.md` — the PATH-resolved binary on user machines may come from npm global install
...
- CI workflows from `tasks/backlog/github.md` — this task only creates the extension artifacts the CI will consume (the `vsce publish` step is in that task's release workflow)
